...
import { i as getSql, t as authMiddleware } from "./middleware-b_CfhXXs.mjs";
import { a as SEED_REVIEWS, i as SEED_PHOTOS, o as SEED_VIDEOS, t as DEFAULT_SETTINGS } from "./content-C7zs6QT1.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/queries-S2LddNz6.js
...
async function requireAdmin(sql, userId) {
	const admins = await sql`select user_id as "userId" from site_admins`;
	if (admins.length === 0) {
		await sql`insert into site_admins (user_id) values (${userId})`;
		return;
	}
	if (!admins.some((a) => a.userId === userId)) throw new Error("This sign-in is not allowed to update the site.");
}
